        ...
        case Jason.decode(body) do
          ...
          {:ok, %{"results" => [result | _]}} ->
            ...
            if result_country && !country_matches?(result_country, country_code) do
              Logger.warning(
                "GeocodingService: Country mismatch for #{city}: expected #{country_code}, got #{result_country}"
              )
              ...
              if lat && lon do
                Logger.info(
                  "GeocodingService: Successfully geocoded #{city}, #{country_code} to #{lat}, #{lon}"
                )
